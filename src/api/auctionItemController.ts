...
/** 此处后端没有提供注释 GET /inProgressAuctionItem/${param0} */
export async function getInProgressAuctionItem(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.getInProgressAuctionItemParams,
  options?: { [key: string]: any }
) {
  const { id: param0, ...queryParams } = params
  return request<API.ApiResponseAuctionItem>(`/inProgressAuctionItem/${param0}`, {
    method: 'GET',
    params: { ...queryParams },
    ...(options || {}),
  })
